...
import Master from '../Components/Student/Navbar';
import axios from 'axios';
import swal from 'sweetalert'

function StudentPrivateRoutes({...rest}){
    ...
    useEffect(()=>{
        axios.get('/api/authenticated_student').then(res=>{
            if(res.status==200)
            {
        ...
        return (
            <nav className="navbar navbar-expand-lg navbar-dark bg-dark shadow sticky-top">
                <div className="container-fluid">
                    <Link className="navbar-brand" to="/student">CompaStudent <span className="fs-6 text-primary">Loading</span></Link>
