...
const UpdateProgram =(props)=>{
    ...
    const programUpdate=(e)=>{
        ...
        axios.put(`/api/update_program/${id}`,data).then(res=>{
            if(res.data.status===200)
            {   
                swal("Success",res.data.message,"success");
                history.push('/admin/view_programs');
            }
            else  if(res.data.status===404 || res.data.status===202)
            {
    ...
    useEffect(()=>{
        let flag=0;
        (async () => {
            await axios.get(`/api/view_program/${id}`).then(res=>{
                if(res.data.status===200)
                {
                    setProgram(res.data.program);
                    setYearSelected(res.data.program.group.year);
                    flag++;
                }
    ...
    else if(loading)
    {
        return (
            <div className="container px-4 py-4 text-primary">
                <span className="h1 my-4">Update Program </span>
    ...
    return(
        <div className="container px-4">
            <h1 className="my-4 d-none d-md-block">Update Program</h1>
            <i className="my-4 fa fa-edit fa-2x d-md-none text-primary"></i>

            <form onSubmit={programUpdate} className="my-5">
                <Link className="btn btn-sm btn-primary px-4 m-1 ms-auto" to="/admin/view_programs" role="button"><i className="fa fa-chevron-left"></i></Link>
                ...
                <div className="border bg-light">
                    <div className="tab-pane fade show active card-body border pt-5" id="info" role="tabpanel" aria-labelledby="info-tab">
                        ...
                        <div className="form-floating mb-3 offset-md-1 col-md-10">
                            ...
                            <span className="text-danger">{errorList.day}</span>
                        </div>
                        ...
                        <div className="form-floating mb-3 offset-md-1 col-md-10">
                            ...
                            <span className="text-danger">{errorList.time_id}</span>
                        </div> 

                        <div className="form-floating mb-3 offset-md-1 col-md-10">
                            <input onChange={handleInput} value={programInput.notice?programInput.notice:"No Thing"} name="notice" className="form-control" id="inputName" placeholder="Enter Notice" />
                            <label htmlFor="inputName">Notice</label>
                            <span className="text-danger">{errorList.notice}</span>
                        </div> 
                        ...
                        <div className="form-floating mb-3 offset-md-1 col-md-10">
                            ...
                            <span className="text-danger">{errorList.hall_id}</span>
                        </div>
                        ...
                        <div className="form-floating mb-3 offset-md-1 col-md-10">
                            <select name="subject_id" onChange={handleInput} defaultValue={programInput.subject_id} className="form-select py-3 mt-3" aria-label="Default select example">
                                ...
                                {selectSubject(yearSelected)}
                            </select>
                            <span className="text-danger">{errorList.subject_id}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="row mt-2">
                    <div className="d-flex justify-content-between flex-wrap flex-sm-nowrap">
                        ...
                        <button type="submit" className="btn btn-primary py-3 col-12 col-sm-6" style={{margin:"1px"}}>Update Program</button>
                    </div>
