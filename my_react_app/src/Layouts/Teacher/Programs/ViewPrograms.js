...
const ViewPrograms =()=>{
    ...
    [idTarget,setIdTarget]=useState(-100);
    ...
    handleSearch=(e)=>{
        ...
        setSearchTable(<SearchedTable role="teacher" handleCopy={handleCopy} idTarget={idTarget} deleteProgram={deleteProgram} inSearchPrograms={inSearchPrograms} whatShow={whatShow} searchKey={e.target.value}/>);
    },
    ...
    const handleCopy=(targetId)=>{
        setIdTarget(targetId);
        swal("Success","Program "+targetId+" Ready, Go To Your Programs To Add Swap Request","success");
    }
    ...
    useEffect(()=>{
        ...
        (async() => {
            await axios.get(`/api/view_programs`).then(res=>{
                ...
                await axios.get(`/api/teacher_programs_mine`).then(res=>{
                    if(res.data.status===200 )
                    {
        ...
        {
            console.log(parseInt(document.getElementById("hiddenInput").value));
            setIdTarget(parseInt(document.getElementById("hiddenInput").value));
        }
    },[searchKey,document.getElementById("hiddenInput")?document.getElementById("hiddenInput").value:null]);
    ...
    {   
        programsTable=programs.map((program,idx)=>{
            ...
            return(
                <tr key={idx}>
                    <td onClick={()=>handleCopy(program.id)}  style={{cursor:"pointer"}} className="py-3 text-center">
        ...
        myProgramsTable=myPrograms.map((myProgram,idx)=>{ 
            return(
                <tr key={idx}>
                    ...
                    <td className="py-2 text-center">{myProgram.group.year.name}</td>
                    <td className="py-2 text-center">{myProgram.group.name}</td>
                    ...
                    <td className="py-2 text-center">
                        <Link className="btn btn-sm btn-dark m-1" to={`/teacher/update_program/${myProgram.id}`} title="Refresh " role="button"><i className="fa fa-edit"></i></Link>
                        <button onClick={(e)=>deleteProgram(e,myProgram.id)} type="button" className="btn btn-sm btn-danger m-1"><i className="fa fa-trash-alt"></i></button>
                        <Link className="btn btn-sm btn-primary m-1" to={`/teacher/swap_program/${myProgram.id}.${idTarget}`} title="Swap" role="button"><i className="fa fa-retweet"></i></Link>
                    </td>
                </tr>
            )
        });
                                ...
                                <tr className="bg-dark text-light">
                                    ...
                                    <th className="text-center" scope="col"> 
                                        <div className="border border-light p-1 px-2 d-inline-block rounded text-light">
                                            <i className="fa fa-arrows-alt-v"></i><span>  {myPrograms.length}     <Link className="btn btn-sm btn-light" to="/teacher/add_program" role="button"><i className="fa fa-plus"></i></Link> </span>
                                        </div> 
                                    </th>
                                </tr>   
